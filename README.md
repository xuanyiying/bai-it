...
配上一个 AI API key（Gemini、OpenAI、DeepSeek 等都行），手动掰句就升级成 AI 深度分析：告诉你这句话是什么句式、为什么难读、怎么学会用类似的句式写作。
...
3. **手动掰句** — 对看不懂的句子点触发按钮
4. **配 API key（可选）** — 点插件图标 → 更多设置 → 设置 Tab，选一个 AI 提供商，填入 API key，解锁深度分析
...
- **API key 只存本地**：你的 key 只存在你的浏览器里，插件直接从你的浏览器调用 AI API，不经过任何中间服务器
