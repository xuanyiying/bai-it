 ...
 * @returns 单词数组
 */
export function parseSentence(text: string): string[] {
  const matches = text.match(/\b[a-zA-Z][a-zA-Z'-]*[a-zA-Z]\b|[a-zA-Z]{3,}\b/g);
  return matches || [];
}
 ...
 * @returns WordInfo 数组
 */
export function annotateWordsMobile(words: string[]): WordInfo[] {
